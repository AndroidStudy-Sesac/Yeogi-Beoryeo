...
- 홈 표시 항목 설정을 열면 선택한 분류를 먼저 보여주고, 건전지 관련 검색 별칭을 보강했습니다.
- 넓은 창과 낮은 가로 창에서 홈 본문 폭과 여백을 조정했습니다.
- 지도 API와 최근 위치 캐시의 비치명 오류를 기존 오류 안내와 복구 동작을 유지하면서 기록합니다.
- CI 로그에서 빌드 경고를 유형별로 분류하고 발생 횟수를 요약합니다.

### Fixed

- 창 크기가 바뀌면 분리배출 분류의 접힘 개수를 다시 계산합니다.
- ‘명륜 1가’, ‘을지로 1가’처럼 공백을 넣은 법정동 검색을 보정했습니다.
