...
class CollectionSpotMapViewModel @Inject constructor(
    ...
    private var lastRetryableSearchRequest: MapSearchRetryRequest? = null
        ...
        _uiState.update {
            it.copy(
                ...
                hasSearched = if (shouldCancelSpotSearch) {
                    ...
                    (it.searchFailure != null || it.errorMessageResId != null) &&
                    it.spots.isEmpty()
                ) {
                    false
                } else {
    ...
    fun clearRegionBackCurrentLocationSheetHidden() {
        _uiState.update {
            it.copy(shouldKeepCurrentLocationSheetHiddenAfterRegionBack = false)
        }
    }
    ...
    private suspend fun searchByKeywordInternal(
        ...
        startKeywordSearchLoading()
        lastRetryableSearchRequest = MapSearchRetryRequest.Keyword(
            keyword = keyword,
        ...
        runCatching {
            ...
            updateSpotFailure(
                failure = MapSearchFailures.fromThrowable(throwable),
            )
        }
    }
    ...
    fun searchByCurrentLocation() {
        ...
        lastRetryableSearchRequest = MapSearchRetryRequest.CurrentLocation
        _uiState.update {
            it.copy(
                searchKeyword = EMPTY_SEARCH_KEYWORD,
                searchFailure = null,
    ...
    fun searchByMapCenter(coordinate: Coordinate) {
        ...
        currentLocationLoadingJob?.cancel()
        cancelSpotSearchJob()
        lastRetryableSearchRequest = MapSearchRetryRequest.MapCenter(coordinate)
        spotSearchJob = viewModelScope.launch {
            val searchStartedAtNanos = System.nanoTime()
            mapSearchTimingLogger.log(
    ...
    fun retrySpotSearch() {
        if (uiState.value.isLoading || uiState.value.searchFailure == null) return

        when (val request = lastRetryableSearchRequest) {
            is MapSearchRetryRequest.Keyword -> {
                currentLocationRefreshJob?.cancel()
                currentLocationLoadingJob?.cancel()
                cancelSpotSearchJob()
                _uiState.update {
                    it.copy(searchKeyword = request.keyword)
                }
                spotSearchJob = viewModelScope.launch {
                    searchByKeywordInternal(
                        keyword = request.keyword,
                        selectedRegionCandidate = request.selectedRegionCandidate,
                        searchKeywordsOverride = request.searchKeywordsOverride,
                    )
                }
            }

            MapSearchRetryRequest.CurrentLocation -> searchByCurrentLocation()

            is MapSearchRetryRequest.MapCenter -> searchByMapCenter(request.coordinate)
        ...
        _uiState.update {
            it.copy(
                ...
                shouldKeepCurrentLocationSheetHiddenAfterRegionBack =
                    it.shouldKeepCurrentLocationSheetHiddenAfterRegionBack &&
                        it.searchMode == MapSearchMode.CURRENT_LOCATION &&
    ...
    private fun updateSpotFailure(failure: MapSearchFailure) {
        currentLocationLoadingJob?.cancel()
        originalSpots = emptyList()
    ...
    private suspend fun searchByCurrentLocationInternal(
        ...
        if (showLoading) {
            startCurrentLocationSearchLoading(searchGeneration)
        }
    ...
    private suspend fun searchByFreshCurrentLocation(
        coordinate: Coordinate,
        preservePreviousResultOnFailure: Boolean,
        searchGeneration: Int,
        allowCachedResult: Boolean,
    ...
    private fun RecentCurrentLocationSpotCacheEntry.isWithinDistanceFrom(
        coordinate: Coordinate,
    ): Boolean {
        val distanceMeter = calculateDistanceMeterUseCase(
            from = searchCoordinate,
            to = coordinate,
        )

        return distanceMeter <= RECENT_LOCATION_CACHE_MAX_DISTANCE_METER
    }
    ...
    private fun startCurrentLocationSearchLoading(searchGeneration: Int) {
        currentLocationLoadingJob?.cancel()
        ...
        currentLocationLoadingJob = viewModelScope.launch {
            delay(CURRENT_LOCATION_LOADING_DELAY_MILLIS)

            if (!canApplyCurrentLocationResult(searchGeneration)) return@launch
...
private sealed interface MapSearchRetryRequest {
    data class Keyword(
        val keyword: String,
        val selectedRegionCandidate: MapRegionSearchCandidate?,
        val searchKeywordsOverride: List<String>?,
    ) : MapSearchRetryRequest

    data object CurrentLocation : MapSearchRetryRequest
